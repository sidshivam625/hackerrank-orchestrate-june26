...
class PostProcessor:
    ...
    def assemble_output_row(
        ...
        # 7. If evidence standard not met, override claim_status
        ...
        # 8. valid_image: False if all images failed quality check
        valid_image = ctx.valid_images_count > 0
    ...
    def _check_evidence_standard(
        ...
        Rule-based override takes precedence over VLM's assessment.
        ...
        # Check pre-computed image quality flags
        blocking_flags = set(ctx.image_quality_flags)

        # If VLM says evidence is met and no blocking quality issues → trust it
        if result.evidence_standard_met and not (
            blocking_flags & {"blurry_image"}  # Only blurry is a hard blocker
        ):
            return result.evidence_standard_met, None

        # If all images are blurry → evidence NOT met
        if "blurry_image" in blocking_flags:
            all_blurry = ctx.valid_images_count == 0
            if all_blurry:
                return (
                    False,
                    "All submitted images are too blurry to verify the claimed damage.",
                )

        # Default: defer to VLM assessment
        return result.evidence_standard_met, None
    ...
    def _normalise_image_ids(raw_ids: str, available_ids: List[str]) -> str:
        ...
        # Validate each ID exists (basic check — img_N format)
        valid = [i for i in ids if i in available_ids or i.startswith("img_")]
        return ";".join(valid) if valid else "none"
